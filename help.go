// help.go
// Назначение: Система справки и документации редактора.
// Предоставляет локализованные тексты помощи на русском и английском языках,
// включая описание горячих клавиш, режимов работы и примеров использования.
...
func printUsageRU() {
	...
	fmt.Println("  -a, --auto        	Включить режим автодополнения с использованием ИИ")
	fmt.Println("                    	Использует LLM для предложения вариантов завершения кода при нажатии Tab")
	fmt.Println("  -i, --input string	Входной файл или директория для LLM")
	fmt.Println("			для потокового режима и для режима редактора")
	fmt.Println()
...
func printUsageEN() {
	...
	fmt.Println("  -a, --auto        	Enable AI-powered auto-complete mode")
	fmt.Println("                    	Uses LLM to suggest code completions when pressing Tab")
	fmt.Println("  -i, --input string	Input file or directory for LLM")
	fmt.Println("			for streaming mode and for editor mode")
	fmt.Println()
